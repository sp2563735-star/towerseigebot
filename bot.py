...
async def send_main_menu(context, game, p, edit_message=None):
    ...
    if submitted_moves:
        ...
        chosen = ', '.join(f"{MOVES[a['move_id']].get('emoji','')} {MOVES[a['move_id']]['name']}" for a in state['moves']) if state['moves'] else 'none'
...
async def send_alliance_menu(context, game, p, edit_message=None):
    ...
    if p.alliance_submitted:
        ...
        names = [f"{ALLIANCE_MOVES[m['move_id']].get('emoji','')} {ALLIANCE_MOVES[m['move_id']]['name']}" for m in p.alliance_moves_pending]
    ...
    for mid, mv in ALLIANCE_MOVES.items():
        ...
        if mid == "bhoomi_domain":
            if mid in pending_ids:
                label = f"{mv.get('emoji','')} Boundless Tower Domain (active)"
                move_buttons.append(InlineKeyboardButton(label, callback_data="ally_na"))
            elif a.bhoomi_forced_rest:
                label = f"{mv.get('emoji','')} Boundless Tower Domain (resting)"
                move_buttons.append(InlineKeyboardButton(label, callback_data="ally_na"))
            elif a.bhoomi_streak >= 2:
                label = f"{mv.get('emoji','')} Boundless Tower Domain (cooldown)"
                move_buttons.append(InlineKeyboardButton(label, callback_data="ally_na"))
            else:
                label = f"{mv.get('emoji','')} Boundless Tower Domain ({a.bhoomi_streak}/2)"
                move_buttons.append(InlineKeyboardButton(label, callback_data=f"ally_move:bhoomi_domain"))
        elif mid in pending_ids:
            label = f"{mv.get('emoji','')} {mv['name']} (used)"
            move_buttons.append(InlineKeyboardButton(label, callback_data="ally_na"))
...
CLASS_INFO = {
    ...
    "Healer": (
        ...
        "Use the Healer when you want to survive, empower troops, or recover from heavy damage."
    ),
}
...
async def cb_soldiers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "bonus_menu":
        ...
        if not avail:
            ...
            return
        move_buttons = [InlineKeyboardButton(f"Restore {MOVES[m].get('emoji','')} {MOVES[m]['name']}", callback_data=f"bonus_pick:{m}") for m in avail]
    ...
    if data.startswith("bonus_pick:"):
        ...
        p.bonus_restores -= 1
        await query.answer(f"Restored {mv.get('emoji','')} {mv['name']}!")
        if p.bonus_restores > 0:
            avail = [mid for mid in NORMAL_MOVE_IDS if p.move_uses.get(mid, 0) < MOVES[mid]["max_uses"]]
            if avail:
                move_buttons = [InlineKeyboardButton(f"Restore {MOVES[m].get('emoji','')} {MOVES[m]['name']}", callback_data=f"bonus_pick:{m}") for m in avail]
...
async def _send_sacrifice_menu(message_or_query, p, uid, edit=False):
    ...
    move_buttons = [InlineKeyboardButton(f"{MOVES[m].get('emoji','')} {MOVES[m]['name']}", callback_data=f"sac:{m}") for m in sacrificeable]
    buttons = _rows(move_buttons, 2)
    if len(state["chosen"]) == 1:
        buttons = [[InlineKeyboardButton("Confirm escape", callback_data="sac_confirm")]]
    parts = []
    for m in state['chosen']:
        mv = MOVES[m]
        parts.append(f"{mv.get('emoji','')} {mv['name']}")
    text = f"Sacrifice 1 move to break free. Chosen: {', '.join(parts) or 'none'}"
    if edit:
